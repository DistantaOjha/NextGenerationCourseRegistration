...
//print $placeVal . '-----' . $_SESSION['username'];
    ...
    if($courseRes != FALSE){
      ...
      while($courseRow = $courseRes->fetch()){       
        ...
        if($sectionRes != FALSE){
          ...
          $sectionDivs = $sectionDivs . '<tr><th><b>Section Letter</b></th><th><b>Faculty</b></th><th><b>Time</b></th><th><b>Days</b></th><th><b>Time</b></th></tr>';
          while($sectionRow = $sectionRes->fetch()){
            $sectionID = $sectionRow['sectionID'];
            $insID = $sectionRow['instructorID'];
            $insRes = $db->query("SELECT * FROM Instructors WHERE instructorID = $insID;");
            if($insRes != FALSE){
              $insRow = $insRes->fetch();
              $insName = $insRow['lname'] . ", " . $insRow['fname'];            
              $sectionStr = "<tr><td>".$sectionRow['sectionLetter']."</td><td>".$insName."</td>";
              $sectionStr = $sectionStr ."<td>".$sectionRow['time']."</td><td>".$sectionRow['days']."</td><td>".$sectionRow['seats']."</td>";
              $sectionStr = $sectionStr .'<td><a href = "addToShoppingCart.php?placeVal='.$placeVal. '&sectionID='.$sectionID.'"><button>Select Section</button></a></td></tr>';
